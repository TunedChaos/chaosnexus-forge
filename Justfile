...
# Materialize Tauri icons when the working tree still has Git LFS pointer stubs.
#
# workspace CI workflow `.ci/workflows/chaosnexus-release-builds.yml`
# sets GIT_LFS_SKIP_SMUDGE=1 and checkout lfs:false; it also hydrates icons from
# This recipe hydrates icon assets for
# local / Justfile-driven builds. It does not unset GIT_LFS_SKIP_SMUDGE.
ensure-tauri-icons:
    #!/usr/bin/env bash
    set -euo pipefail
    icons_dir="{{justfile_directory()}}/src-tauri/icons"
    assets_dir="{{justfile_directory()}}/assets"
    is_lfs_pointer() {
        [[ -f "$1" ]] || return 1
        local first
    ...
    if [[ "${#pointers[@]}" -eq 0 ]]; then
        echo "  -> Tauri icons are real files (not Git LFS pointers)"
        exit 0
    fi
    echo "  -> Git LFS pointer stubs in icon assets (GIT_LFS_SKIP_SMUDGE checkout):"
    printf '       %s\n' "${pointers[@]}"
    if ! git lfs version >/dev/null 2>&1; then
        echo "error: git-lfs is required to hydrate src-tauri/icons before tauri build" >&2
        echo "   Install git-lfs, then: git lfs pull" >&2
        echo "   Or rely on build.rs icns restore when pointers remain." >&2
        exit 1
    fi
    ...
    include="$(rel_include "$icons_dir" "src-tauri/icons/**")"
    include="${include},$(rel_include "$assets_dir" "assets/**")"
    echo "  -> git lfs pull --include=${include} (leaving GIT_LFS_SKIP_SMUDGE as-is)"
    git lfs pull --include="$include"
    still=0
    for f in "${pointers[@]}"; do
        if is_lfs_pointer "$f"; then
            echo "error: still an LFS pointer after pull: $f" >&2
            still=1
        fi
    done
    if [[ "$still" -ne 0 ]]; then
...
forge-release-linux: check-deps-linux ensure-tauri-icons
    #!/usr/bin/env bash
    set -euo pipefail
    ver="{{PKG_VERSION}}"
    out="{{ARTIFACTS_LINUX}}"
...
forge-flatpak: check-deps-linux ensure-tauri-icons
    #!/usr/bin/env bash
    set -euo pipefail
    ver="{{PKG_VERSION}}"
    out="{{ARTIFACTS_LINUX}}"
...
forge-cross-windows: check-deps-cross ensure-tauri-icons
    #!/usr/bin/env bash
    set -euo pipefail
    ver="{{PKG_VERSION}}"
    out="{{ARTIFACTS_CROSS}}"
...
forge-cross-macos: check-deps-cross ensure-tauri-icons
    #!/usr/bin/env bash
    set -euo pipefail
    ver="{{PKG_VERSION}}"
    out="{{ARTIFACTS_CROSS}}"

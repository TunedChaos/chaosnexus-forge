...
pub fn crucible_binary_candidates(app: &AppHandle) -> Vec<PathBuf> {
    ...
    crate::suite_paths::push_suite_bin_candidates(&mut candidates, "chaosnexus-crucible");
...
pub fn crucible_start(
    ...
    for bin in crucible_binary_candidates(&app) {
        ...
        let gguf_file = settings.crucible_gguf_file.clone().or_else(|| {
            if model_id == "TunedChaos/ChaosNexus_Tuned_v1-GGUF" {
                Some("ChaosNexus_Tuned_v1-Q4_K_M.gguf".to_string())
            } else {
                None
            }
        });
        let gguf_line = gguf_file
            .as_ref()

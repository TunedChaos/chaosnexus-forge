...
/// Suite bundle next to Forge, sibling debug/release build dirs, then
/// `chaosnexus-anvil` on `PATH`.
pub fn chaoswrench_binary_candidates(app: &AppHandle) -> Vec<PathBuf> {
    ...
    crate::suite_paths::push_suite_bin_candidates(&mut candidates, "chaosnexus-anvil");
...
/// Ordered candidates for the ChaosNexus Codex binary (docs MCP/CLI).
pub fn codex_binary_candidates(_app: &AppHandle) -> Vec<PathBuf> {
    let mut candidates: Vec<PathBuf> = Vec::new();
    if let Ok(custom) = std::env::var("CHAOSNEXUS_CODEX_BIN") {
        candidates.push(PathBuf::from(custom));
    }
    crate::suite_paths::push_suite_bin_candidates(&mut candidates, "chaosnexus-codex");
    if let Ok(exe) = std::env::current_exe() {
        if let Some(dir) = exe.parent() {
            for profile in ["debug", "release"] {
                candidates.push(
                    dir.join("../../../chaosnexus-codex/target")
                        .join(profile)
                        .join("chaosnexus-codex"),
                );
            }
        }
    }
    candidates.push(PathBuf::from("chaosnexus-codex"));
    candidates
}

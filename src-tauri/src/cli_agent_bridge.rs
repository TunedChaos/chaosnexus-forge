...
/// Returns default preset agent profiles (Goose, Antigravity, Echo probe — no shell -c).
#[tauri::command]
pub fn list_agent_profiles() -> Vec<AgentProfile> {
    vec![
        ...
        // Deliberately no sh -c profile: arbitrary prompt-as-shell is RCE via the webview.
        AgentProfile {
            id: "echo".to_string(),
            name: "Echo (safe probe)".to_string(),
            binary: "echo".to_string(),
            args: vec!["{prompt}".to_string()],
            env: HashMap::new(),
            description: "Safe argv-only probe; not a shell.".to_string(),
        },
    ]
}

fn builtin_profile_by_id(id: &str) -> Option<AgentProfile> {
    list_agent_profiles().into_iter().find(|p| p.id == id)
}

/// Spawns a managed external CLI agent process, streaming stdout and stderr via Tauri events.
/// `profile_id` must match a builtin profile (caller-supplied binary/args/env ignored).
#[tauri::command]
pub async fn spawn_cli_agent(
    ...
    let profile = builtin_profile_by_id(&profile_id).ok_or_else(|| {
        format!(
            "Unknown or disallowed agent profile id '{profile_id}'. Use list_agent_profiles()."
        )
    })?;

    if prompt.chars().any(|c| c == '\n' || c == '\r' || c == '\0') {
        return Err("prompt contains disallowed control characters".into());
    }
    ...
    // Prefer direct exec of allowlisted binary. Optional bwrap binds only the
    // working directory (not the entire host filesystem).

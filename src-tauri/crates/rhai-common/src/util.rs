...
mod tests {
    ...
    #[test]
    fn glob_rule_includes_and_excludes() {
        let rule = GlobRule::new(["**/*.rhai"], ["**/vendor/**"]).unwrap();
        assert!(rule.is_match("src/main.rhai"));
        assert!(!rule.is_match("vendor/lib.rhai"));
        assert!(!rule.is_match("src/main.rs"));
    }

    #[test]
    fn normalize_str_decodes_percent() {
        let out = normalize_str("file:///tmp/hello%20world.rhai");
        assert!(out.contains("hello world.rhai"));
    }
}

...
export class WorkbenchState {
  ...
  async probeDefaultWorkspaces() {
    const defaults: string[] = [
      // Suite AppImage / portable installs: get_suite_plugins_dir inserted first below.
      "../chaosnexus-scripts/plugins",
      "./chaosnexus-scripts/plugins",
      "../chaosnexus-anvil/plugins",
    ];
    try {
      const suitePlugins = await invoke<string | null>("get_suite_plugins_dir");
      if (suitePlugins) {
        defaults.unshift(suitePlugins);
      }
    } catch {
      // Dev / web mock: command may be absent.
    }

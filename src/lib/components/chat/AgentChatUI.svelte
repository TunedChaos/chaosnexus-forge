  ...
  <div class="flex-none flex flex-col theme-bg-header theme-border-b px-3 py-2 gap-2">
    ...
    <div class="flex items-center justify-between gap-2 min-w-0">
      ...
      <div class="flex items-center gap-1.5 shrink-0">
        ...
        {#if appSettings.modelStatus && (appSettings.modelStatus.startsWith("Not cached") || appSettings.modelStatus.includes("model missing") || appSettings.modelStatus.startsWith("Download failed"))}
          <span
            class="text-[11px] theme-text-muted max-w-[14rem] truncate"
            data-testid="crucible-model-missing"
            title={appSettings.modelStatus}
          >
            Model missing — Settings → Models
          </span>
        {/if}

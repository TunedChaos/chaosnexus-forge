...
<script lang="ts">
  ...
  import AnvilMcpPanel from "$lib/components/AnvilMcpPanel.svelte";
  import SkillsRulesPanel from "$lib/components/SkillsRulesPanel.svelte";
  ...
  // Left sidebar view: plugins, Mesh, Anvil MCP, or Skills/Rules.
  let leftTab = $state<"plugins" | "mesh" | "anvil-mcp" | "skills">("plugins");
  ...
  // Persist agent chat dock width when the splitter moves.
  $effect(() => {
    void agentChat.dockWidth;
    agentChat.save();
  });
  ...
  onMount(() => {
    ...
    if (typeof window !== "undefined") {
      ...
      hook.mcp = mcp;
      hook.agentChat = agentChat;
      hook.crucible = crucible;
...
>
  ...
  <div class="flex-1 flex overflow-hidden min-h-0 relative">
    ...
    <div style="width: {sidebarWidth}px;" class="flex-none h-full overflow-hidden flex flex-col">
      <!-- Sidebar view tabs -->
      <div class="flex-none flex theme-bg-header theme-border-b text-[10px] font-bold uppercase tracking-wider">
        <button
        ...
        <button
          ...
          data-testid="sidebar-tab-mesh"
          onclick={() => (leftTab = "mesh")}
        >
          Mesh
        </button>
        <button
          class="flex-1 py-1.5 transition-colors cursor-pointer {leftTab === 'anvil-mcp'
            ? 'theme-text-accent theme-bg-sidebar border-b-2 theme-border-accent'
            : 'theme-text-muted hover:theme-text-main border-b-2 border-transparent'}"
          aria-pressed={leftTab === "anvil-mcp"}
          data-testid="sidebar-tab-anvil-mcp"
          onclick={() => (leftTab = "anvil-mcp")}
        >
          Anvil MCP
        </button>
        <button
          class="flex-1 py-1.5 transition-colors cursor-pointer {leftTab === 'skills'
            ? 'theme-text-accent theme-bg-sidebar border-b-2 theme-border-accent'
            : 'theme-text-muted hover:theme-text-main border-b-2 border-transparent'}"
          aria-pressed={leftTab === "skills"}
          data-testid="sidebar-tab-skills"
          onclick={() => (leftTab = "skills")}
        >
          Skills
        </button>
      </div>
    ...
    <!-- Right docked Agent Chat -->
    {#if agentChat.enabled && agentChat.open && agentChat.mode === "docked"}
      <Splitter
        min={280}
        max={720}
        bind:value={agentChat.dockWidth}
        reverse={true}
      />
      <div
        style="width: {agentChat.dockWidth}px;"
        class="flex-none h-full overflow-hidden flex flex-col min-h-0"
...
{#if agentChat.enabled && agentChat.open && agentChat.mode === "float"}
  <FloatingDraggable
    bind:x={agentChat.floatX}
    bind:y={agentChat.floatY}
    bind:width={agentChat.floatWidth}
    bind:height={agentChat.floatHeight}
    onMove={() => agentChat.save()}
  >
    <AgentChatUI
      onClose={() => {
        agentChat.open = false;
